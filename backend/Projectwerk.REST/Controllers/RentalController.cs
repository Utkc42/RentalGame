...
using Projectwerk.REST.Helpers;
...
public class RentalController : ControllerBase
{
    ...
    public RentalController(ILogger<RentalController> logger, RentalRepository rentalRepository,
        UserRepository userRepository, GameRepository gameRepository, IMapper mapper)
    {
        _logger = logger;
    ...
    public async Task<IActionResult> GetRentals([FromQuery] bool includeDeleted, [FromQuery] int? userId)
    {
        ...
        var currentUserIntId = Convert.ToInt32(currentUserId);

        // Check if the current user is an admin
        var isAdmin = User.IsInRole("admin") || User.IsInRole("Admin");

        // If a non-admin user tries to access rentals for another user, forbid the request
        if (!isAdmin && userId.HasValue && userId != currentUserIntId) return Forbid();
        ...
        if (!includeDeleted) rentals = rentals.Where(r => !r.IsDeleted);
        ...
        if (!rentalDTOs.Any()) return NoContent();
    ...
    public async Task<IActionResult> GetMyRentals([FromQuery] bool includeDeleted = false)
    {
        ...
        // Retrieve rentals for the current user
        var rentals = await _rentalRepository.GetRentalsForUser(currentUserIntId);
    ...
    public async Task<IActionResult> CheckInGame(int rentalId)
    {
        try
        {
            ...
            if (rental.IsDeleted) return BadRequest("The game has already been checked in.");

            // Set the rental status to checked in
            rental.IsDeleted = true;
            rental = RentalService.HandleLateReturn(rental);
            await _rentalRepository.Update(rental);

            // Retrieve the associated game from the repository
            var game = await _gameRepository.GetById((int)rental.GameId);
            if (game == null) return NotFound("Associated game not found.");
    ...
    public async Task<IActionResult> ExtendRental(int id)
    {
        try
        {
            ...
            if (rental.NumberOfExtensions >= 2) return BadRequest("Maximum number of extensions reached.");

            rental.NumberOfExtensions++; // Increment the number of extensions
            rental.EndRentalPeriod = rental.EndRentalPeriod.AddDays(7); // Extending by 1 week

            // Update the rental in the repository
            await _rentalRepository.Update(rental);

            return Ok(rental); // Return the updated rental object
        }
        catch (Exception ex)
        {
