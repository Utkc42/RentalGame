...
namespace Projectwerk.REST.Helpers {
	public class RentalService {
		public static decimal CalculateLateFee(DateTime returnDate, DateTime dueDate, int lateCount) {
			...
			int lateWeeks = (int)Math.Ceiling(lateDuration.TotalDays / 7) - 1; // Exclude the first week
			decimal lateFee = 0;
			...
			if (lateWeeks > 0) {
				if (lateCount < 3) {
					...
					lateFee = 50 + lateWeeks * 0.4m * 50; // €50 base fee after third late return + 40% increase per extra week
				}
			}
		...
		public static Rental HandleLateReturn(Rental rental) {
			...
			DateTime dueDate = rental.EndRentalPeriod;
			int lateCount = rental.User.LateReturnCount;

			if (returnDate > dueDate) {
				decimal lateFee = CalculateLateFee(returnDate, dueDate, lateCount);
